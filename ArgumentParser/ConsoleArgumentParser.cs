...
namespace ZhanC.ArgumentParser
{
    /// <summary>
    /// 
    /// </summary>
    public class ConsoleArgumentParser
    {
        ...
        // Ctor.
        public ConsoleArgumentParser(Type argType, bool supportPartialName = false)
        {
        ...
        private void SetPropertyValue(object o, string propertyName, object value)
        {
            ...
            {
                var existingValue = property.GetValue(o) as string[];
                if (existingValue != null)
                {
                    existingValue = existingValue.Concat(new string[] { value?.ToString() }).ToArray();
                    property.SetValue(o, existingValue, null);
                }

                else
                {

                    property.SetValue(o, new string[] { value!.ToString() }, null);
                }
            ...
            while (index < tokens.Length)
            {
                ...
                try
                {
                    ...
                    if (match.IsFlag)// a flag argument
                    {
                        ...
                        {
                            bool isBool = bool.TryParse(tokens[index], out val);
                            if (isBool)
                            {
                                _nextIsBoolValue = true;
                            }
                        }
                        SetPropertyValue(t, match.PropertyName, val);
